...
def avatar_updated_handler(sender, user, avatar, **kwargs):
    ...
    msg = EmailMultiAlternatives(
        ...
        body='Volunteer with email ' + volunteer.user.email + ' changed their avatar image.  Please approve (' + volunteer.get_approve_avatar_url() + ') or reject (' + volunteer.get_reject_avatar_url() + ').',
        from_email=settings.DEFAULT_FROM_EMAIL,
        to=[v for k,v in settings.ADMINS]
    )
    msg.attach_alternative('<h1>Is this avatar okay?</h1><img src="' + avatar.avatar_url(400) + '"><h2><a href="' + volunteer.get_approve_avatar_url() + '">Approve</a></h2><h2><a href="' + volunteer.get_reject_avatar_url() + '">Reject</a></h2>', 'text/html')
