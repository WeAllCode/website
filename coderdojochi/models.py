...
class Session(CommonInfo):
    ...
    def get_mentor_orders(self):
        orders = MentorOrder.objects.filter(session=self, is_active=True,).order_by("mentor__user__last_name")

        return orders

    def get_checked_in_mentor_orders(self):
        orders = MentorOrder.objects.filter(session=self, is_active=True, check_in__isnull=False).order_by(
    ...
    def get_current_students(self, checked_in=None):
        if checked_in is not None:
            if checked_in:
                orders = Order.objects.filter(is_active=True, session=self).exclude(check_in=None).values("student")
            else:
                orders = Order.objects.filter(is_active=True, session=self, check_in=None).values("student")
        else:
            orders = Order.objects.filter(is_active=True, session=self).values("student")
...
class Equipment(CommonInfo):
    ...
    condition = models.CharField(max_length=255, choices=EQUIPMENT_CONDITIONS,)
