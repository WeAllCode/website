...
def student_register_link(context, student, session):
    ...
    if orders.count():
        ...
        if not student.is_within_age_range(
            ...
            title = "Limited event."
            message = (
                f"Sorry, this class is limited to {session.gender_limitation}s between {session.minimum_age} and"
                f" {session.maximum_age} this time around."
            )

            button_href = (
                f'data-trigger="hover" data-placement="top" data-toggle="popover" title="" data-content="{message}"'
