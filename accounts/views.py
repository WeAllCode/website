...
class MyAccountView(TemplateView):
    def get(self, request):
        ...
        if request.user.role == 'mentor':
            return my_account_mentor(request)

        if request.user.role == 'guardian':
            return my_account_guardian(request)
...
@login_required
def my_account_mentor(request, template_name='account/home_mentor.html'):
    highlight = (
...
@login_required
def my_account_guardian(request, template_name='account/home_guardian.html'):
    highlight = (
        request.GET['highlight'] if 'highlight' in request.GET else False
    )
